...
const oldUiEnd = "          )}";
// We know it ends with `Select File` button and `</div>` and `) : (`.
const regex = /\{generatedQuestions\.length === 0 \? \([\s\S]*?<\/[bB]utton>[\s\S]*?<\/div>[\s\S]*?\) : \(/;
...
// 4. Also we need to display the passage correctly if it's there.
const oldQuestionRender = "<p style={{ color: '#1e293b', fontSize: '0.95rem', lineHeight: '1.6', marginBottom: '1rem', whiteSpace: 'pre-wrap' }}>{q.question}</p>";
const newQuestionRender = \`{q.passage && (
...
if (!code.includes("q.passage &&")) {
  code = code.replace(oldQuestionRender, newQuestionRender);
}

fs.writeFileSync('src/app/teacher/create-test/page.tsx', code, 'utf8');
console.log("Updated Create Test successfully!");
