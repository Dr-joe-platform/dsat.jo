...
import { getUserResults, AppUser, TestResult, getTestBanks, AdminTestBank, getMiniQuizzes, MiniQuiz } from '@/lib/db';
import { ArrowLeft, User, Phone, CheckCircle, BarChart2, Star, AlertTriangle, Loader2 } from 'lucide-react';
import { useAuth } from '@/lib/auth-context';
import { filterResultsBySubject, filterItemsBySubject } from '@/lib/subject-filter';
...
export default function TeacherStudentProfile() {
  ...
  const [teacherTests, setTeacherTests] = useState<(AdminTestBank | MiniQuiz)[]>([]);
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        if (snap.exists()) {
          setStudent({ uid: snap.id, ...snap.data() } as AppUser);

          // Get all student results and filter by teacher's subject
          const r = await getUserResults(uid);
          const filteredResults = filterResultsBySubject(r, appUser.teacherSubject);
          setResults(filteredResults);

          // Get teacher's tests
          const [banks, quizzes] = await Promise.all([
            getTestBanks(appUser.uid, 'teacher'),
            getMiniQuizzes(appUser.uid)
          ]);
          
          const combined: (AdminTestBank | MiniQuiz)[] = [...banks, ...quizzes];
          const filteredTests = filterItemsBySubject(combined as any, appUser.teacherSubject);
          setTeacherTests(filteredTests as any);
        }
      } catch (err) {
    ...
    if (uid && appUser) loadData();
  }, [uid, appUser]);

  if (loading) {
    return <div style={{ padding: '3rem', textAlign: 'center', color: '#64748b' }}><Loader2 size={24} className="spin" style={{ margin: '0 auto 0.5rem', animation: 'spin 1s linear infinite' }} />Loading student profile...</div>;
  }
  ...
  // Unsolved tests are tests owned by the teacher that the student has access to but hasn't solved.
  // We'll just show all teacher's tests that the student hasn't solved yet.
  const unsolvedTests = teacherTests.filter(t => !solvedTestIds.has(t.id!));
  ...
  return (
    <div style={{ maxWidth: '1200px', paddingBottom: '3rem' }}>
      <button onClick={() => router.push('/teacher')} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'none', border: 'none', color: '#6366f1', fontWeight: '700', fontSize: '0.9rem', cursor: 'pointer', marginBottom: '1.5rem' }}>
      ...
      <div style={{ background: '#fff', borderRadius: '1rem', padding: '2rem', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)', border: '1px solid #e2e8f0', display: 'flex', gap: '2rem', flexWrap: 'wrap', marginBottom: '2rem' }}>
        ...
        <div style={{ flex: 1, minWidth: '250px' }}>
          <h1 style={{ fontSize: '1.75rem', fontWeight: '800', color: '#0f172a', marginBottom: '0.25rem' }}>{student.displayName}</h1>
          <p style={{ color: '#64748b', fontSize: '1rem', marginBottom: '1rem' }}>{student.email}</p>
          <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#475569', fontSize: '0.9rem' }}>
              <Phone size={16} /> Student: 
              {student.phone ? <a href={`https://wa.me/${student.phone.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer" style={{ color: '#25d366', fontWeight: 'bold', textDecoration: 'none' }}>{student.phone}</a> : 'N/A'}
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#475569', fontSize: '0.9rem' }}>
              <Phone size={16} /> Parent: 
              {student.parentPhone ? <a href={`https://wa.me/${student.parentPhone.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer" style={{ color: '#25d366', fontWeight: 'bold', textDecoration: 'none' }}>{student.parentPhone}</a> : 'N/A'}
            </div>
          </div>
          <div style={{ marginTop: '1.5rem' }}>
            <span style={{ background: '#f8fafc', border: '1px solid #e2e8f0', color: '#475569', padding: '0.375rem 0.875rem', borderRadius: '1rem', fontSize: '0.8rem', fontWeight: '700' }}>
              Viewing {appUser?.teacherSubject || 'All'} Subject Data
            </span>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '1.5rem', marginBottom: '2rem' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, #3b82f6, #1d4ed8)', color: '#fff', borderRadius: '1rem', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '0.5rem', boxShadow: '0 10px 15px -3px rgba(59,130,246,0.3)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.9rem', fontWeight: '700', opacity: 0.9 }}><BarChart2 size={18} /> Avg {appUser?.teacherSubject ? appUser.teacherSubject.charAt(0).toUpperCase() + appUser.teacherSubject.slice(1) : 'Total'} Score</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
                  ...
                  {results.sort((a, b) => (b.completedAt?.seconds || 0) - (a.completedAt?.seconds || 0)).map((r, idx) => {
                    ...
                    return (
                      <tr key={r.id || idx} style={{ borderBottom: '1px solid #f1f5f9' }}>
                        ...
                        <td style={{ padding: '1rem', fontWeight: '800', color: '#3b82f6', fontSize: '0.85rem' }}>{relevantScore}</td>
        ...
        {/* Unsolved Tests (Your Tests) */}
        <div style={{ background: '#fff', borderRadius: '1rem', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
          <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid #e2e8f0', background: '#f8fafc', fontWeight: '800', color: '#0f172a', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <AlertTriangle size={18} color="#f59e0b" /> Pending Tests ({unsolvedTests.length})
          </div>
          <div style={{ maxHeight: '400px', overflowY: 'auto' }}>
            {unsolvedTests.length === 0 ? (
              <div style={{ padding: '2rem', textAlign: 'center', color: '#94a3b8' }}>Student has solved all your tests!</div>
            ) : (
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: '#f1f5f9' }}>
                    <th style={{ padding: '0.75rem 1rem', textAlign: 'left', fontSize: '0.75rem', color: '#64748b' }}>Test Name</th>
                    <th style={{ padding: '0.75rem 1rem', textAlign: 'left', fontSize: '0.75rem', color: '#64748b' }}>Type</th>
                  </tr>
                </thead>
                <tbody>
                  {unsolvedTests.map(t => (
                    <tr key={t.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                      <td style={{ padding: '1rem', fontWeight: '600', color: '#0f172a', fontSize: '0.85rem' }}>{'name' in t ? t.name : (t as any).title || 'Test'}</td>
                      <td style={{ padding: '1rem', color: '#64748b', fontSize: '0.8rem', textTransform: 'capitalize' }}>{'isMiniQuiz' in t && t.isMiniQuiz ? 'Mini Quiz' : 'Full Test'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
